...
pub fn compute_cost_usd_with_card(
    ...
    (input_tokens as f64) * r.input_per_token
        ...
        + (cache_creation_tokens as f64) * r.cache_creation_per_token
...
mod tests {
    ...
    fn sonnet_basic_cost() {
        ...
        let cost = compute_cost_usd_with_card("claude-sonnet-4-6", 1_000_000, 100_000, 0, 0, &fallback_card());
    ...
    fn haiku_with_cache_read() {
        ...
        let cost = compute_cost_usd_with_card("claude-haiku-4-5", 0, 0, 0, 1_000_000, &fallback_card());
    ...
    fn opus_output_only() {
        ...
        let cost = compute_cost_usd_with_card("claude-opus-4-8", 0, 1_000_000, 0, 0, &fallback_card());
    ...
    fn fable_rates() {
        ...
        let cost = compute_cost_usd_with_card("claude-fable-5", 500_000, 500_000, 0, 0, &fallback_card());
        assert!((cost - 30.0).abs() < 1e-9, "expected $30.00, got ${}", cost);
    }

    #[test]
    fn cache_creation_at_1_25x_input_rate() {
        // Sonnet: 1M cache_create @ $3.75/MTok + 1M cache_read @ $0.30/MTok = $4.05
        let cost = compute_cost_usd_with_card("claude-sonnet-4-6", 0, 0, 1_000_000, 1_000_000, &fallback_card());
        assert!((cost - 4.05).abs() < 1e-9, "expected $4.05, got ${}", cost);
    }

    #[test]
    fn unknown_model_falls_back_to_sonnet() {
        let cost_unknown = compute_cost_usd_with_card("claude-unknown-99", 1_000_000, 0, 0, 0, &fallback_card());
        let cost_sonnet = compute_cost_usd_with_card("claude-sonnet-4-6", 1_000_000, 0, 0, 0, &fallback_card());
